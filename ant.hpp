...
class Ant {
    ...
    public:
        Ant(std::shared_ptr<std::default_random_engine> engine_in, CircleObstacle nest) : engine(engine_in) {
            ...
            // Spawn ants on the edge of the nest
        ...
        std::vector<std::shared_ptr<Marker>> get_markers_fov(std::vector<std::shared_ptr<Marker>> markers, float speed, int antenna_depth, float fov) {
            // Finds markers in fov of the ant
            ...
            std::copy_if(markers.begin(), markers.end(), std::back_inserter(markers_in_fov),
                         [this, speed, antenna_depth, ant_fov](auto& marker) -> bool
                         { return marker->distance(x, y) < speed * antenna_depth && marker->distance(x, y) > 0.05 && marker->in_fov(ant_fov); });
        ...
        void move(CircleObstacle food, CircleObstacle nest, std::vector<std::shared_ptr<Obstacle>> walls, const std::vector<std::shared_ptr<Marker>>& markers, float speed, int& nfood, int antenna_depth, float fov, int n_antennas, std::pair<float, float> bias_ratio) {
            ...
            for (int i = 0; i < n_antennas; i++) {
                float offset_rad = i * fov / (n_antennas - 1) - fov / 2;
                float next_x = x + std::cos(rad + offset_rad) * speed, next_y = y + std::sin(rad + offset_rad) * speed;
            ...
            for (int offset_ind = 0; offset_ind < offsets.size(); offset_ind++) {
                std::pair<float, float> bias = get_antenna_bias(offsets.at(offset_ind), food, nest, walls,
                                                                markers_in_fov, speed);
                biases.push_back(bias);
            }
            auto [min_marker, max_marker] = std::minmax_element(biases.begin(), biases.end(),
                                                                [](auto &l, auto &r) { return l.first < r.first; });
            auto [min_collision, max_collision] = std::minmax_element(biases.begin(), biases.end(),
                                                                      [](auto &l, auto &r) {
            ...
            for (int i = 0; i < offsets.size(); i++) {
                float branch_bias = bias_ratio.first * (biases.at(i).first - min_marker->first) / marker_range +
                                    bias_ratio.second * (biases.at(i).second - min_collision->second) / collision_range;
            ...
            float next_x = x + std::cos(rad) * speed;
            float next_y = y + std::sin(rad) * speed;
